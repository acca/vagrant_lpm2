...
** DONE Note sul currying
   - "Trucco" standard per esprimere funzioni in piu` variabili
     #+BEGIN_EXAMPLE
     f(x,y) ⇒ f′(x), dove f() ritorna un numero, mentre f′() ritorna una funzione!
     #+END_EXAMPLE
   - Notare che in ML una funzione si applica ai suoi argomenti senza usare parentesi e che l’applicazione di funzione associa a sinistra
     - Tramite currying possiamo codificare funzioni a n argomenti usando fn, prima si applica la funzione ad a, poi si applica il risultato (che e` una funzione) a b
       #+BEGIN_EXAMPLE
       (fnx=>fny=>x∗x+y∗y)ab= ((fn x => fn y => x ∗ x + y ∗ y) a) b       
       #+END_EXAMPLE
